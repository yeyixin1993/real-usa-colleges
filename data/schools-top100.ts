...
const cityCoordinates: Record<string, { lat: number; lng: number }> = {
  ...
  'Villanova|PA': { lat: 40.0379, lng: -75.3433 },
  ...
  'Williamstown|MA': { lat: 42.7120, lng: -73.2037 },
  'Swarthmore|PA': { lat: 39.9021, lng: -75.3499 },
  'Wellesley|MA': { lat: 42.2968, lng: -71.2924 },
  'Brunswick|ME': { lat: 43.9145, lng: -69.9653 },
  'Northfield|MN': { lat: 44.4583, lng: -93.1616 },
  'Claremont|CA': { lat: 34.0967, lng: -117.7198 },
  'Davidson|NC': { lat: 35.5012, lng: -80.8490 },
  'Haverford|PA': { lat: 40.0134, lng: -75.2944 },
  'Poughkeepsie|NY': { lat: 41.7004, lng: -73.9210 },
  'Lexington|VA': { lat: 37.7840, lng: -79.4428 },
  'Clinton|NY': { lat: 43.0484, lng: -75.3785 },
  'Waterville|ME': { lat: 44.5520, lng: -69.6317 },
  'Middlebury|VT': { lat: 44.0153, lng: -73.1673 },
  'Grinnell|IA': { lat: 41.7430, lng: -92.7224 },
  'Hamilton|NY': { lat: 42.8270, lng: -75.5443 },
  'Northampton|MA': { lat: 42.3251, lng: -72.6412 },
  'Bryn Mawr|PA': { lat: 40.0198, lng: -75.3155 },
...
function makeSchool(seed: Seed): School {
  ...
  const cityKey = `${seed.city}|${seed.stateCode}`;
  const cityPoint = cityCoordinates[cityKey];
  const baseLat = (cityPoint?.lat ?? meta.lat) + jitter(seed.slug, 0.08);
  const baseLng = (cityPoint?.lng ?? meta.lng) + jitter(`${seed.slug}-lng`, 0.08);
