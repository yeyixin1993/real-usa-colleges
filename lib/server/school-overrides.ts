...
type SchoolOverride = Partial<School>;

type OverrideMap = Record<string, SchoolOverride>;

const OVERRIDES_PATH = path.join(process.cwd(), 'data', 'school-overrides.json');
...
export async function getAllSchoolOverrides(): Promise<OverrideMap> {
...
export async function getSchoolOverride(slug: string): Promise<SchoolOverride | null> {
  const all = await getAllSchoolOverrides();
  return all[slug] ?? null;
}

export async function upsertSchoolOverride(slug: string, patch: SchoolOverride) {
  const all = await getAllSchoolOverrides();
...
export function applySchoolOverride(base: School, override?: SchoolOverride): School {
